# ══════════════════════════════════════════════════════════════════════════════
#  DocMind — Production RAG Chat  |  app.py
#  Senior review fixes applied:
#
#  BUG 1  — render_input_bar: `triggered = (send or bool(query))` fires on
#            EVERY rerun because `query` is always non-empty after first type.
#            Even without clicking Send, every rerun re-queued the same text.
#            FIX: gate strictly on `send` (button click) OR a form submit flag;
#            never trigger on bare text presence.
#
#  BUG 2  — last_query dedup is broken: after process_pending clears
#            pending_query and reruns, the input widget still holds the old
#            value (Streamlit preserves widget state across reruns unless the
#            key changes). So `query.strip() != last_query` is False and it
#            won't re-trigger — BUT if the user types the exact same question
#            again it silently ignores it.
#            FIX: use a submit counter (int) as the dedup key, not text
#            comparison. Increment on submit, compare in process_pending.
#
#  BUG 3  — is_processing flag left True if rag_query raises an unhandled
#            exception outside its own try/except (e.g. network timeout at
#            LangChain layer before our code runs). UI freezes permanently.
#            FIX: wrap entire process_pending body in try/finally to guarantee
#            is_processing = False.
#
#  BUG 4  — build_vectorstore: temp file not cleaned up if PyPDFLoader raises
#            (the unlink is not in a finally block — it's after the call).
#            FIX: use try/finally around the whole build.
#
#  BUG 5  — retrieve_context: Chroma's similarity_search_with_relevance_scores
#            can return scores outside [0,1] depending on the distance metric
#            (cosine similarity may return negative values for unrelated docs).
#            Clamping to [0,1] before threshold comparison prevents phantom
#            "high confidence" results from wrapping around.
#            FIX: clamp score = max(0.0, min(1.0, raw_score)).
#
#  BUG 6  — chat_history grows unboundedly in `process_pending` before trim:
#            the new HumanMessage is appended, then rag_query reads the full
#            history (including the new message), then AIMessage is appended,
#            THEN we trim. So trim happens after the LLM call, not before.
#            For a fast-talking user this means history can spike before trim.
#            FIX: trim BEFORE passing history to rag_query.
#
#  BUG 7  — render_upload_panel: `can_index` checks doc_name != uploaded_file
#            .name but uploaded_file is a new UploadedFile object on every
#            rerun (Streamlit re-creates it). The name comparison is correct
#            but the button is disabled if the same file is re-selected after
#            a failed index attempt (vectorstore is None but doc_name still
#            matches from a previous partial run).
#            FIX: only block re-index if vectorstore is not None AND names match.
#
#  BUG 8  — Session state init uses mutable defaults ([] and None shared
#            across all sessions in the same Python process if using the dict
#            literal approach). While Streamlit's session_state is per-session,
#            the _DEFAULTS dict at module level holds references; if anyone
#            mutates st.session_state["messages"] in-place those are safe, but
#            the init pattern `st.session_state[k] = v` assigns the same list
#            object reference. Two sessions could share the same list.
#            FIX: always use factory functions (copy/deepcopy) for mutable defaults.
#
#  BUG 9  — is_err detection uses `safe_content.startswith("⚠")` but
#            html.escape() on "⚠ Error..." escapes nothing (⚠ is not an HTML
#            special char), so this works — BUT if the error message itself
#            contains `<`, `>`, `&` those get double-escaped when rendered.
#            We escape the content for injection safety but then the displayed
#            text shows &lt; etc. for error tracebacks.
#            FIX: escape content correctly; detect error on the RAW content
#            before escaping, not after.
#
#  BUG 10 — render_input_bar sets last_query = clean then immediately reruns.
#            On the rerun, the text_input widget still holds the old value
#            (because key="query_input" and Streamlit restores it). So after
#            the answer is shown and the user clears the field and types
#            something NEW that happens to equal last_query (impossible in
#            practice but worth noting), it would silently drop. More critically:
#            the input field NEVER CLEARS after sending because we don't reset
#            the widget key. The user sees their previous query sitting in the
#            box, which is confusing UX.
#            FIX: rotate the input widget key after each submission so Streamlit
#            creates a fresh empty input on the next render.
# ══════════════════════════════════════════════════════════════════════════════
